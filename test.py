...
# parser = argparse.ArgumentParser(description="What should my code get?")
                ...
                for mu in MU:
                    ...
                    for it in range(n_iters):  
                        ...
                        while (counter < max_loops):
                            ...
                            if((counter == max_loops - 1) and (terminate == False)):
                                if (len(survivors) == 1 and not np.any(survivors[0])):
                                    print("stuck at all zeros at loop ", counter)
                                    terminate = True   

                                if (terminate == False):
                                    #print(n)
                                    #time.sleep(3)
                                    if (suspects == survivors):
                                        print("check more loops")
                                        counter = max_loops - 1
                                        print("stuck after checking more loops")
                                        terminate == True
                                    else:
                                        if (n == 0):
                                            suspects = survivors
                                            counter = max_loops - 10000
                                            n = 1
                                        else:
                                            print("check more loops")
                                            counter = max_loops
                                            terminate = True
                                            print("We don't really know")
                    ...
                    print('p_fail = ', p_fail)
                    new_row = {'G': g, 'S':ss, 'dim':d, 'damp':dd, 'MU': mu, 'mean_n_loop':np.mean(n_loops), 'p_fail':p_fail}
...
df.to_csv("dim-S-" + str(args.dim) + "_damp1.csv", index=False)
#df = df.pivot('S', 'dim', 'p_fail')
